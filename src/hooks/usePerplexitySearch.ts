...
export const usePerplexitySearch = (puterChat?: PuterChatOptions) => {
  ...
  const runSearch = useCallback(
    async (
      ...
      try {
        ...
        // Lesson Mode -> Puter Claude (if enabled)
        if (lessonMode && puterChat?.enabled && window.puter?.ai?.chat) {
          const model = puterChat.model || 'claude-sonnet-4';
          const message = await window.puter.ai.chat(query, { model });
          const content =
            typeof message === 'string'
              ? message
              : (message as any)?.message ?? (message as any)?.text ?? JSON.stringify(message);
        ...
        setSearches((prev) => {
          ...
          if (existing) {
            updated.set(id, {
              ...
              result: {
                success: false,
                error: language === 'ar' ? 'فشل الاتصال بالخادم' : 'Connection failed',
              },
              isLoading: false,
            });
          }
    ...
    [toast, language, lessonMode, puterChat]
  );

  const search = useCallback(
    async (query: string, type: 'general' | 'news' | 'smart', images: string[] = []) => {
      const id = generateId();
